mod2_server <- function(input, output, session) {
  ...
  observeEvent(input$filein, {
    ...
    validate(need(!is.null(data$guide), "Please upload your data"),
  ...
  observeEvent(input$testin, {
    ...
    validate(need(!is.null(data$test), "Please upload your data"),
  ...
  observeEvent(input$anno_in, {
    ...
    validate(need(!is.null(data$anno), "Please upload your data"),
             need(is.data.frame(data$anno), data$anno))
  }, priority = 40)
  
  observeEvent(input$run_method, {
    waitress <-
      Waitress$new("#module2-results", theme = "overlay-percent")
    for (i in 1:10) {
  ...
  observeEvent(input$sample_button, {
    data$guide <-
      input_checking(read.csv("simData_guide.csv", stringsAsFactors = T))
    data$test <-
      input_checking(read.csv("simData_test.csv", stringsAsFactors = T))
  ...
  output$mod2_upload_component1 <- renderUI({
    fileInput(
      "filein",
      label = p(strong("Guide Data")),
      accept = c(".csv"),
      placeholder = data$mod2_upload_msg1
    )
  })
  ...
  output$mod2_upload_component2 <- renderUI({
    fileInput(
      "testin",
      label = p(strong("Test Data")),
      accept = c(".csv"),
      placeholder = data$mod2_upload_msg2
    )
  })
  ...
  observeEvent(input$showtest, {
    showModal(modalDialog(
      ...
      if (is.null(data$test)) {
        ...
        DT::dataTableOutput("testview", height = "70vh")
      }
    ))
  })
  ...
  observeEvent(input$desc_modal, {
    showModal(
      modalDialog(
        ...
        else if (input$box_plot_switch == "Per-Peptide") {
          uiOutput("box_plotly")
        }
        else if (input$box_plot_switch == "Per-Metric") {
  ...
  check_mark_green <-
    HTML(
      '<span class="glyphicon glyphicon glyphicon-ok-sign" style="font-size:3rem;color:green;"></span>'
    )
  check_mark <-
    HTML(
      '<span class="glyphicon glyphicon glyphicon-ok-circle" style="font-size:3rem;"></span>'
    )
  ...
  output$upload_mark <- renderUI({
    ...
    if (uploaded) {
      ...
      data$L <- 1
      data$U <-
        nrow(data$guide) / length(levels(data$guide$Precursor))
  ...
  output$metric_mark <- renderUI({
    if (length(input$user_selected_metrics) >= 1) {
      check_mark_green
    }
    else{
      check_mark
  ...
  output$selectMeanSD <- renderUI({
    lapply(input$user_selected_metrics,
           function(x) {
             fluidRow(column(4, paste(x, ":")),
                      column(4,
                             numericInput(
                               paste0("selectMean@", x), "mean", value = 1
                             )),
                      column(4,
                             numericInput(
                               paste0("selectSD@", x), "standard deviation", value = 1
                             )))
           })
  })
  ...
  output$peptideThresholdYellow <- renderUI({
    ...
    numericInput(
      'threshold_peptide_yellow',
      '',
      value = threshold_peptide_red - 1,
      min = 1,
      max = threshold_peptide_red,
      step = 1
    )
  })
  ...
  output$metricThresholdYellow <- renderUI({
    ...
    validate(need(!is.null(numOfMetrics), "loading..."),
             need(!is.null(threshold_metric_red), "loading..."))
    
    numericInput(
      'threshold_metric_yellow',
      '',
      value = threshold_metric_red ,
      min = 0,
      max = threshold_metric_red,
      step = 1
    )
  })
  ...
  output$XmR_tabset <- renderUI({
    validate(
      ...
      need(
        !is.null(input$user_selected_metrics),
        "Please first select metrics and create a decision rule"
      )
    )
                     ...
                     tabPanel(
                       ...
                       renderPlotly(
                         render.QC.chart(
                           ...
                           plot.method = "XmR",
                           normalization = FALSE,
                           y.title1 = "Individual Value",
                           y.title2 = "Moving Range",
                           selectMean = input[[paste0("selectMean@", x)]],
                           selectSD = input[[paste0("selectSD@", x)]],
                     ...
                     tabPanel(
                       ...
                       renderPlotly(
                         render.QC.chart(
                           ...
                           plot.method = "CP",
                           normalization = TRUE,
                           y.title1 = "Change point for mean",
                           y.title2 = "Change point for variation",
                           selectMean = input[[paste0("selectMean@", x)]],
                           selectSD = input[[paste0("selectSD@", x)]],
                           guidset_selected = is_guidset_selected
                         )
  ...
  output$box_plotly <- renderUI({
    ...
    get_plot_output_list <- function(input_n) {
      guide_box_plots <-
        peptide_box.plot(
          guideset,
          data.peptides = boxplot_peps,
          data.metrics = input$user_selected_metrics,
          ret_obj_list = T
        )
      test_box_plots <-
        peptide_box.plot(
      ...
      plot_output_list <- lapply(1:input_n, function(i) {
        ...
        plot_output_object <-
          wellPanel(h3(strong(levels(
            testset$Precursor
          )[[i]])), br(),
          fluidRow(
            column(6, h4("Guide Set"), renderPlotly({
              guide_box_plots[[i]]
            })),
            column(6, h4("Test Set"), renderPlotly({
              test_box_plots[[i]]
            }))
          ))
      })
  ...
  output$heat_map <- renderPlot({
    validate(
      ...
      need(
        !is.null(data$df$AcquiredTime),
        "To view heatmaps, the dataset should include Acquired Time column."
      )
    )
    
    prodata <- data$df
    peptideThresholdRed <-
      (as.numeric(input$threshold_peptide_red)) / 100
    peptideThresholdYellow <-
      (as.numeric(input$threshold_peptide_yellow)) / 100
    
    
    if (is.null(prodata$AcquiredTime))
      return(NULL)
  ...
  output$ml_heat_map <- renderUI({
    ...
    if (data$run == 0) {
      ...
      trained_model <-
          MSstatsQC.ML.trainR(
            guide.set,
            use_simulation = user_sim_bool,
            sim.size = user_sim_size,
            guide.set.annotations = anno.set
          )
      }
      else {
        trained_model <-
          MSstatsQC.ML.trainR(guide.set,
                              use_simulation = user_sim_bool,
                              sim.size = user_sim_size)
      }
      ...
      ML_plots <-
        MSstatsQC.ML.testR(test.set, guide.set, rf_model = trained_model)
      ...
      plot_output_list <- lapply(1:length(ML_plots), function(i) {
        ...
        plot_output_object <- wellPanel(fluidRow(renderPlot({
          ML_plots[[i]]
        })))
      })
  ...
  output$mod2report <- downloadHandler(
    ...
    content = function(file) {
      ...
      params <- list(box_plots = data$box_plots)
